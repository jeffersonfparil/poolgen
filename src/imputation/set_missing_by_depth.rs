...
impl GenotypesAndPhenotypes {
    pub fn set_missing_by_depth(&mut self, min_depth_set_to_missing: &f64) -> io::Result<&mut Self> {
...
mod tests {
    ...
    fn test_set_missing() {
        ...
        println!("frequencies_and_phenotypes.chromosome[0..5]={:?}", &frequencies_and_phenotypes.chromosome[0..5]);
        println!("frequencies_and_phenotypes.position[0..5]={:?}", &frequencies_and_phenotypes.position[0..5]);
        println!("frequencies_and_phenotypes.allele[0..5]={:?}", &frequencies_and_phenotypes.allele[0..5]);
        ...
        println!("frequencies_and_phenotypes.coverages={:?}", frequencies_and_phenotypes.coverages);
        assert_eq!(frequencies_and_phenotypes.intercept_and_allele_frequencies[(0, 1)].is_nan(), true);
        assert_eq!(frequencies_and_phenotypes.intercept_and_allele_frequencies[(0, 4)], 1.0/21.0);
        assert_eq!(frequencies_and_phenotypes.intercept_and_allele_frequencies[(3, 13)], 2.0/10.0);
        assert_eq!(frequencies_and_phenotypes.intercept_and_allele_frequencies[(3, 18)].is_nan(), true);
        assert_eq!(frequencies_and_phenotypes.intercept_and_allele_frequencies[(3, 18)].is_nan(), true);
        // assert_eq!(0, 1);
    }
}
