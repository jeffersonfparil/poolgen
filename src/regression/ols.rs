...
fn ols(x_matrix: &DMatrix<f64>, y_matrix: &DMatrix<f64>) -> io::Result<(DMatrix<f64>, DMatrix<f64>, DMatrix<f64>)> {
...
mod tests {
    ...
    fn test_ols() {
        ...
        // Inputs
        let x: DMatrix<f64> = DMatrix::from_row_slice(5, 3, &[1.0, 0.4, 0.1,
                                                                                    1.0, 0.2, 0.1,
                                                                                    1.0, 0.3, 0.2,
                                                                                    1.0, 0.4, 0.3,
                                                                                    1.0, 0.5, 0.5]);
        let y: DMatrix<f64> = DMatrix::from_row_slice(5, 2, &[2.0, 0.5,
                                                                                    1.0, 0.2,
                                                                                    2.0, 0.5,
                                                                                    4.0, 0.0,
                                                                                    5.0, 0.5]);
        let counts: DMatrix<u64> = DMatrix::from_row_slice(5, 3, &[4, 1, 5,
                                                                                      2, 1, 7, 
                                                                                      3, 2, 5,
                                                                                      4, 3, 3,
                                                                                      5, 5, 0]);
        ...
        let locus_counts = LocusCounts{chromosome: "Chromosome1".to_owned(), position: 12345, alleles_vector: vec!["A".to_owned(), "T".to_owned(), "D".to_owned()], matrix:counts};
        ...
        assert_eq!(k1, 2);
        assert_eq!(k1, k2);
        assert_eq!((expected_output1[(0,0)]*100.0).round(), (beta[(0,0)]*100.0).round()); // estimated betas for the first phenotype
        ...
        assert_eq!((expected_output2[(0,0)]*100.0).round(), (var_beta[(0,0)]*100.0).round()); // estimated beta variances for the first phenotype
        ...
        assert_eq!((expected_output3[(0,0)]*100.0).round(), (pval[(0,0)]*100.0).round()); // estimated pvalues for the first phenotype
        assert_eq!((expected_output3[(1,0)]*100.0).round(), (pval[(1,0)]*100.0).round());
        assert_eq!((expected_output3[(2,0)]*100.0).round(), (pval[(2,0)]*100.0).round());
        assert_eq!((expected_output4[(0,0)]*100.0).round(), (beta[(0,1)]*100.0).round()); // estimated betas for the second phenotype
        assert_eq!((expected_output4[(1,0)]*100.0).round(), (beta[(1,1)]*100.0).round());
        assert_eq!((expected_output4[(2,0)]*100.0).round(), (beta[(2,1)]*100.0).round());
        assert_eq!((expected_output5[(0,0)]*100.0).round(), (var_beta[(0,1)]*100.0).round()); // estimated beta variances for the second phenotype
        assert_eq!((expected_output5[(1,0)]*100.0).round(), (var_beta[(1,1)]*100.0).round());
        assert_eq!((expected_output5[(2,0)]*100.0).round(), (var_beta[(2,1)]*100.0).round());
        assert_eq!((expected_output6[(0,0)]*100.0).round(), (pval[(0,1)]*100.0).round()); // estimated pvalues for the second phenotype
        assert_eq!((expected_output6[(1,0)]*100.0).round(), (pval[(1,1)]*100.0).round());
        assert_eq!((expected_output6[(2,0)]*100.0).round(), (pval[(2,1)]*100.0).round());
