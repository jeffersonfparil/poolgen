...
pub fn xpclr_per_window(
    ...
    let mut xpclr_per_pool_per_window: Array2<f64> = Array2::from_elem((n_windows, n), f64::NAN);
    Ok((xpclr_per_pool_per_window, windows_chr, windows_pos))
}
...
pub fn xpclr(
    ...
    let (xpclr_per_pool_per_window, windows_chr, windows_pos) =
        xpclr_per_window(genotypes_and_phenotypes, window_size_bp).unwrap();
    let n = xpclr_per_pool_per_window.ncols();
    let n_windows = xpclr_per_pool_per_window.nrows();
    let vec_xpclr_across_windows = xpclr_per_pool_per_window.mean_axis(Axis(0)).unwrap();
    // Write output
    ...
    for i in 0..n {
        let line = genotypes_and_phenotypes.pool_names[i].to_owned()
            + ","
            + &vec_xpclr_across_windows[i].to_string()
            + ","
            + &xpclr_per_pool_per_window
                .column(i)
                .iter()
