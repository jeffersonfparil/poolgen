...
pub fn fisher(locus_counts: &mut LocusCounts, filter_stats: &FilterStats) -> Option<String> {
    ...
    let mut counts: DMatrix<f64> = DMatrix::from_iterator(
        n,
        p,
        locus_counts.matrix.clone().into_iter().map(|x| *x as f64),
    );
    ...
    for max_i in 0..n {
        for max_j in 0..p {
            for i in 0..n {
                for j in 0..p {
                    max = vec![
                        (row_sums[(i, 0)] - counts.index((i, 0..j)).sum().ceil()) as usize,
            ...
            for inv_j in 0..p {
                for inv_i in 0..n {
                    ...
                    i = n - (inv_i + 1);
                    max = vec![
                        (row_sums[(i, 0)] - counts.index((i, 0..p)).sum().ceil()) as usize,
...
mod tests {
    ...
    fn test_fisher() {
        ...
        // Inputs
        let x: f64 = 5.0;
        let counts_f64: DMatrix<f64> =
            DMatrix::from_row_slice(3, 2, &[0.0, 3.0, 1.0, 5.0, 2.0, 6.0]);
