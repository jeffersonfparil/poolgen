...
fn negative_likelihood_normal_distribution_sigma_and_beta(params: &Vec<f64>, x: &DMatrix<f64>, y: &DVector<f64>) -> f64 {
    let (n, p) = x.shape();
    assert_eq!(n, y.len());
...
impl CostFunction for UnivariateMaximumLikelihoodEstimation {
    ...
    fn cost(&self, p: &Self::Param) -> Result<Self::Output, core::Error> {
        Ok(negative_likelihood_normal_distribution_sigma_and_beta(&p, &self.x, &self.y))
    }
}
...
impl Regression for UnivariateMaximumLikelihoodEstimation {
    ...
    fn estimate_effects(&mut self) -> io::Result<&mut Self> {
        ...
        let res = match Executor::new(cost, solver)
                                    ...
                                    .run() {
                    Ok(x) => x,
                    Err(_) => return Err(Error::new(ErrorKind::Other, "T_T Did not converge or something went terribly wrong!")), // Error occurs when the optimiser MoreThuenteLineSearch moves in the wrong direction
                };
...
mod tests {
    ...
    fn test_ols() {
        // Expected
        let expected_output1 = DVector::from_column_slice(&[-0.7317073161994628, 5.528455290972765, 6.422764230005505]);
        // Inputs
