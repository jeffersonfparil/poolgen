...
fn search_params_to_shapes(params: &Vec<f64>) -> Vec<f64> {
    let shapes = params.into_iter()
                                 .map(|x| UPPER_LIMIT / (1.00 + (-x).exp()) )
                                 .collect::<Vec<f64>>();
    shapes //.into_iter()
    //       .map(|x| {if x < 0.0 { 0.0 
    //                      } else if x > UPPER_LIMIT { UPPER_LIMIT 
    //                      } else if x == f64::INFINITY { UPPER_LIMIT
    //                      } else if x == f64::NEG_INFINITY { 0.0
    //                      } else { x }} )
    //       .collect::<Vec<f64>>()
}
...
fn prepare_freqs_and_qprime(locus_frequencies: &LocusFrequencies, bins: &DMatrix<f64>, q: &DMatrix<f64>, sig: f64, min: f64, max: f64, n: usize, j: usize) -> (f64, DMatrix<f64>, DMatrix<f64>, DMatrix<f64>, DMatrix<f64>, DMatrix<f64>) {
    // let freqs_a: DMatrix<f64> = DMatrix::from_columns(&[locus_frequencies.matrix.column(j)]).add_scalar(SMALL_VALUE); // Add a small value so we don't get negative log-likelihoods
...
pub fn gwalpha(locus_counts_and_phenotypes: &mut LocusCountsAndPhenotypes, filter_stats: &FilterStats) -> Option<String> {
    let (locus_frequencies,
        ...
        max,
        n,
        p) = match prepare_geno_and_pheno_stats(locus_counts_and_phenotypes, filter_stats) {
            Some(x) => x,
            None => return None
    };
    // Prepare output line
    ...
    let (mut p_a, mut _q_prime, mut percs_a, mut percs_a0, mut percs_b, mut percs_b0);
    // Iterate across alleles
    for j in 0..p {
        (p_a, _q_prime, percs_a, percs_a0, percs_b, percs_b0) = prepare_freqs_and_qprime(&locus_frequencies, &bins, &q, sig, min, max, n, j);
        // Optimise
        let h = 1.0;
