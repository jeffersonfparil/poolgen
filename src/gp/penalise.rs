...
pub fn penalise_lasso_like(
    ...
    let (b_hat, lambda) =
        penalised_lambda_path_with_k_fold_cross_validation(x, y, false, "Lasso-like".to_owned(), 0.1, 10)
            .unwrap();
    // println!("##############################");
...
pub fn penalise_ridge_like(
    ...
    let (b_hat, lambda) =
        penalised_lambda_path_with_k_fold_cross_validation(x, y, false, "Ridge-like".to_owned(), 0.1, 10)
            .unwrap();
    // println!("##############################");
    // println!("{:?}: {:?}", function_name!().to_owned(), b_hat);
    Ok((
...
#[function_name::named]
fn ols_iterative_for_penalisation(x: &DMatrix<f64>,
    y: &DMatrix<f64>,
) -> io::Result<(DMatrix<f64>, String)> {
    let (n, p) = x.shape();
    let (n_, m) = y.shape();
...
pub fn penalise_lasso_like_iterative_base(
    ...
    let (b_hat, lambda) =
        penalised_lambda_path_with_k_fold_cross_validation(x, y, true, "Lasso-like".to_owned(), 0.1, 10)
            .unwrap();
    // println!("##############################");
    // println!("{:?}: {:?}", function_name!().to_owned(), b_hat);
    Ok((
...
pub fn penalise_ridge_like_iterative_base(
    ...
    let (b_hat, lambda) =
        penalised_lambda_path_with_k_fold_cross_validation(x, y, true, "Ridge-like".to_owned(), 0.1, 10)
            .unwrap();
    // println!("##############################");
    // println!("{:?}: {:?}", function_name!().to_owned(), b_hat);
    Ok((
