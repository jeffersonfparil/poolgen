    ...
    for GenotypesAndPhenotypes
{
    ...
    fn performance(&self, y_true: &DMatrix<f64>, y_pred: &DMatrix<f64>) -> io::Result<Vec<DVector<f64>>> {
        let (n, m) = y_true.shape();
        let (n_, m_) = y_pred.shape();
        if (n != n_) | (m != m_) {
            return Err(Error::new(ErrorKind::Other, "Observed and predicted phenotypes do not match."))
        }
        ...
        for j in 0..m {
            ...
            mse[j] = (y_true.column(j) - y_pred.column(j)).norm_squared() / f64::powf(max - min, 2.0);
